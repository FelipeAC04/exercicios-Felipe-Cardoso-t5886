...
if (grifinoria.includes('Ron')) { console.log(grifinoria[1].toUpperCase()) }
...
console.log(nome.trim().toUpperCase(), "quantos digitos possui?", nome.trim().length)
...
/*9.
Descrição do problema: Você foi contratado para desenvolver um sistema que organize e distribua missões ninjas em Konoha. 
Cada missão tem um líder ninja, um tipo de missão (A para missões de rank A, B para missões de rank B, C para missões de rank C), 
e uma região de destino (NV para nas vilas, FL para florestas, MO para montanhas). Dependendo da combinação de tipo de missão e região,
 o sistema deve determinar a dificuldade da missão e o pagamento total:
   
- Missões de rank A nas vilas (NV) têm uma dificuldade "Alta" e um pagamento de 5000 ryous, nas florestas (FL) a dificuldade é "Muito Alta" 
e o pagamento é de 7000 ryous, e nas montanhas (MO) a dificuldade é "Extrema" com um pagamento de 10000 ryous.
   
- Missões de rank B nas vilas têm uma dificuldade "Média" e um pagamento de 3000 ryous, nas florestas a dificuldade é "Alta" e o pagamento é de 5000 ryous, 
e nas montanhas a dificuldade é "Muito Alta" com um pagamento de 7000 ryous.
  
- Missões de rank C nas vilas têm uma dificuldade "Baixa" e um pagamento de 1000 ryous, nas florestas a dificuldade é "Média" e o pagamento é de 3000 ryous,
 e nas montanhas a dificuldade é "Alta" com um pagamento de 5000 ryous.
...
let tipomissao = ask.question('qual eh o tipo de missao? A, B ou C ')
let regiao = ask.question('qual o local da missao? NV, FL, MO ')
let valoresVilaAbc = [5000, 3000, 1000]
let valoresFlorestaAbc = [7000, 5000, 3000]
let valoresMontanhaAbc = [10000, 7000, 5000]

if (regiao.toUpperCase() === 'NV') {
    if (tipomissao.toUpperCase() === 'A') { console.log(`a missao tem dificuldade alta, o pagamento total será ${valoresVilaAbc[0]} ryous`) }
    else if (tipomissao.toUpperCase() === 'B') { console.log(`a missao tem dificuldade media, o pagamento total será ${valoresVilaAbc[1]} ryous`) }
    else if (tipomissao.toUpperCase() === 'C') { console.log(`a missao tem dificuldade baixa, o pagamento total será ${valoresVilaAbc[2]} ryous`) }

} else if (regiao.toUpperCase() === 'FL') {
    if (tipomissao.toUpperCase() === 'A') { console.log(`a missao tem dificuldade alta, o pagamento total será ${valoresFlorestaAbc[0]} ryous`) }
    else if (tipomissao.toUpperCase() === 'B') { console.log(`a missao tem dificuldade media, o pagamento total será ${valoresFlorestaAbc[1]} ryous`) }
    else if (tipomissao.toUpperCase() === 'C') { console.log(`a missao tem dificuldade baixa, o pagamento total será ${valoresFlorestaAbc[2]} ryous`) }

} else if (regiao.toUpperCase() === 'MO')
    if (tipomissao.toUpperCase() === 'A') { console.log(`a missao tem dificuldade extrema, o pagamento total será ${valoresMontanhaAbc[0]} ryous`) }
    else if (tipomissao.toUpperCase() === 'B') { console.log(`a missao tem dificuldade muito alta, o pagamento total será ${valoresMontanhaAbc[1]} ryous`) }
    else if (tipomissao.toUpperCase() === 'C') { console.log(`a missao tem dificuldade alta, o pagamento total será ${valoresMontanhaAbc[2]} ryous`) } 
